...
(function ($) {
    ...
    function initNicescroll() {
        if (wind_.width() >= 1200) {
            ...
            let header_dropdown = document.querySelectorAll('.header [data-bs-toggle="dropdown"]')
...
$('.table-responsive').on('hide.bs.dropdown', function () {
    ...
    function menuShowActivePage() {
        if ($('.menu').length) {
            let menu_active_link = $('.menu .menu-body ul li a.active');
    ...
    $('.bg-image').each(function () {
        const item = $(this);
        item.css({'background': 'url(' + item.data('image') + ')'});
        setTimeout(function () {
    ...
    $('.collapsed-menu .menu').hover(function () {
        ...
        let collapsed_menu = $('.collapsed-menu .menu .menu-body');
    ...
    doc_.on('click', '#TableOfContents li a', function () {
        ...
        $('html,body').animate({scrollTop: aTag.offset().top - $('.header').height() - 20}, 300);
    })
