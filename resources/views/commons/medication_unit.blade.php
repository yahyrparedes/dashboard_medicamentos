...
@section('page-title', 'Unidades de Medicamentos')

@section('header-action-button')
    <button class="btn btn-primary btn-icon">
        <i class="bi bi-plus-circle"></i> Unidades de Medicamentos
    </button>
@endsection
        ...
        <table id="invoices" class="table table-custom table-lg">
            ...
            @foreach($department as $data)
                <tr>
                    ...
                    <td>
                        @if($data->is_active)
                            <form method="post" action="{{ route('medication_unit.update', $data->id) }}">
                                @method('PATCH')
                                @csrf
